...
void main() {
  ...
  test('Altered Schema - Changes', () async {
    ...
    var response =
        await new File('test/test_data/user_schema/user.schema.altered.json')
            .readAsString();
  ...
  test('GraphQL generation', () async {
    /// Read the test response from the file.
    var graphQLGenerator = GraphQLGenerators();
    var response = await graphQLGenerator.getSchema(
        "https://turtle-api.herokuapp.com/graphiql", "",
    introspectionQuery: '{"query" : "fragment FullType on __Type { kind name description fields(includeDeprecated: true) '
    ...
    var typesFromResponse =
        getTypeList(graphQLGenerator.getTypesFromResponse(response: response));
    var mutationTypeName =
        graphQLGenerator.getMutationTypeNameFromResponse(response: response);
    var namespace = "T";

    /// Generate the dart code and write it to a file.
    var resultFile =
        await File('test/test_data/turtle_backend.schema.result.txt')
            .writeAsString(GraphQLCodeGenerators().generateDartCode(
                typesFromResponse, namespace, {}, {}, mutationTypeName));
  ...
  tearDown(() async {
    ...
    if (FileSystemEntity.typeSync('test/test_data/turtle_backend.result.txt') !=
    ...
    if (FileSystemEntity.typeSync(
            'test/test_data/user_schema/user.result.txt') !=
        FileSystemEntityType.notFound) {
      var resultFile = await File('test/test_data/user_schema/user.result.txt');
      resultFile.delete();
    }
    ...
    if (FileSystemEntity.typeSync(
            'test/test_data/turtle_backend.schema.result.txt') !=
        FileSystemEntityType.notFound) {
      var resultFile =
          await File('test/test_data/turtle_backend.schema.result.txt');
      resultFile.delete();
